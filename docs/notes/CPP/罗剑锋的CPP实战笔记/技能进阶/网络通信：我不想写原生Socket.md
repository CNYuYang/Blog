...
第一个要说的库是 libcurl，它来源于著名的[curl 项目](curl 项目)，也是 curl 的底层核心。
...
cpr 也支持异步处理，但它内部没有使用 libcurl 的 multi 接口，而是使用了标准库里的 future 和 async（参见[第 14 讲](第 14 讲)），和 libcurl 的实现相比，既简单又好理解。
...
ZMQ 是用 C++ 开发的，但出于兼容的考虑，对外提供的是纯 C 接口。不过它也有很多 C++ 封装，这里我选择的是自带的[cppzmq](cppzmq)，虽然比较简单，但也基本够用了。
...
ZMQ 的用法就是这么简单，但想要进一步发掘它的潜力，处理大流量的数据还是要去看[它的文档](它的文档)，选择合适的工作模式，再仔细调节各种参数。
...
关于 ZMQ 就暂时说到这里，它还有很多强大的功能，你可以阅读[官网](官网)上的教程和指南，里面非常详细地讨论了 ZMQ 的各种模式和要点。
