...
这里我选择的是 MessagePack（[第 15 讲](第 15 讲)），我看重的是它小巧轻便的特性，而且用起来也很容易，只要在类定义里添加一个宏，就可以实现序列化：
...
Summary 类里还有一个特别的统计功能，计算所有图书销量的第一名和最后一名。这用到了 minmax_element 算法（[第 13 讲](第 13 讲)）。又因为比较规则是销量，而不是 ID 号，所以还要用 lambda 表达式自定义比较函数：
...
因为客户端程序比较简单，只是序列化，再用 ZMQ 发送，所以我就不讲了，你可以课下去看[GitHub](GitHub)上的源码，今天我主要讲服务器端。

在 main() 函数开头，首先要加载配置文件，然后是数据存储类 Summary，再定义一个用来计数的原子变量 count（[第 14 讲](第 14 讲)），这些就是程序运行的全部环境数据：
...
有了这个 lambda，现在就可以用 async（[第 14 讲](第 14 讲)）来启动服务循环：
...
搭建符合要求的 Web 服务不是件小事，所以这里为了方便测试，我联动了一下《透视 HTTP 协议》，用那里的 OpenResty 写了个的 HTTP 接口：接收 POST 数据，然后打印到日志里，你可以参考[第 41 讲](第 41 讲)在 Linux 上搭建这个后台服务。

log_cycle 其实就是一个简单的 HTTP 客户端，所以代码的处理逻辑比较好理解，要注意的知识点主要有三个，都是前面讲过的：

- 读取 Lua 配置中的 HTTP 服务器地址和周期运行时间（[第 17 讲](第 17 讲)）；

- JSON 序列化数据（[第 15 讲](第 15 讲)）；

- HTTP 客户端发送请求（[第 16 讲](第 16 讲)）。
...
1. 添加 try-catch，处理可能发生的异常（[第 9 讲](第 9 讲)）；

2. 写一个动态库，用 Lua/Python 调用 C++ 发送请求，以脚本的方式简化客户端测试（[第 17 讲](第 17 讲)）；

3. 把前端与服务器的数据交换格式改成 JSON 或者 ProtoBuf（[第 15 讲](第 15 讲)），然后用工厂类封装序列化和反序列化功能，隔离接口（[第 19 讲](第 19 讲)、[第 20 讲](第 20 讲)）。
