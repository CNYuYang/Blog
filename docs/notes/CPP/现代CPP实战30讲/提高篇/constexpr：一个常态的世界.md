...
以 [[第 13 讲]]([第 13 讲]) 提到的阶乘函数为例，和那个版本基本等价的写法是：
...
变量模板是 C++14 引入的新概念。之前我们需要用类静态数据成员来表达的东西，使用变量模板可以更简洁地表达。**constexpr** 很合适用在变量模板里，表达一个和某个类型相关的编译期常量。由此，type traits 都获得了一种更简单的表示方式。再看一下我们在 [[第 13 讲]]([第 13 讲]) 用过的例子：
...
这段代码利用模板特化（[[第 12 讲]]([第 12 讲]) 、[[第 14 讲]]([第 14 讲])）和 **false_type**、**true_type** 类型（[[第 13 讲]]([第 13 讲])），定义了 **is_pair**，用来检测一个类型是不是 **pair**。随后，我们定义了内联 **constexpr** 变量（本讲）**is_pair_v**，用来简化表达。
...
这段代码使用 SFINAE 技巧（[[第 14 讲]]([第 14 讲])），来检测模板参数 **T** 的对象是否已经可以直接输出到 **ostream**。然后，一样用一个内联 **constexpr** 变量来简化表达。
...
要看一下用到 output_container 的例子，可以回顾 [[第 4 讲]]([第 4 讲]) 和 [[第 5 讲]]([第 5 讲])。
