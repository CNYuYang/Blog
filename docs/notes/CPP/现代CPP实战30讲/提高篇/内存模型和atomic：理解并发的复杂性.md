...
有了原子对象之后，我们可以轻而易举地把 [[第 2 讲]]([第 2 讲]) 中的 **shared_count** 变成线程安全。我们只需要包含 &lt;atomic&gt; 头文件，并把下面这行
...
在 Intel 处理器架构上，唯一可见的重排是多处理器下的写读操作。大力推荐你尝试一下参考资料 **[2]** 中的例子（Windows 和 Linux 下可直接运行；macOS 下需要使用我的[修改版本](修改版本)或备用[下载链接](下载链接)来覆盖下载代码中的 gcc/ordering.cpp），并修改预定义宏。另外一种改法就是把代码中的 **X**、**Y** 的类型改成 **atomic_int**，重排也就消失了。
