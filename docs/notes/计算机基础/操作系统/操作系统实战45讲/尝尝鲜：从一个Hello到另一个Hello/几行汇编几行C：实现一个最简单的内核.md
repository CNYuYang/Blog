...
当然，更先进的[UEFI BIOS](UEFI BIOS)则不同，这里就不深入其中了，你可以通过链接自行了解。
...
这里先提个醒：**C 语言字符串是以 0 结尾的，其字符编码通常是 utf8，而 utf8 编码对 ASCII 字符是兼容的，即英文字符的 ASCII 编码和 utf8 编码是相等的**（关于[utf8](utf8)编码你可以自行了解）。
